...
const highlightLineRange = require(`packages/unisite-gatsby-remark-prismjs/src/directives`);

const fixtures = require(`packages/unisite-gatsby-remark-prismjs/src/__tests__/fixtures`);

const output = highlighted => highlighted.map(({
  code
}) => code).join(`\n`);
...
describe(`highlighting lines`, () => {
  ...
  describe(`with multiline tokens`, () => {
    const highlights = highlightCode(`python`, fixtures.multilineSpanAndHighlight, {}, [2, 3]);
